...
export const CHANGELOG = {
  '10.4.59': [
    'content(2026-Jun): +150 questions — the complete IMA שלב א\' רפואה פנימית קיץ 2026 board exam (18.06.2026), tag 2026-Jun. Corpus 1556 → 1706. Questions transcribed verbatim from the official שאלון booklet (digital-text extraction with a sequential-marker parser that rejects lab-value false markers like ".30K"); the 6 table/matrix questions (Q43 non-thyroidal-illness thyroid panel, Q124 CTEPH right-heart-cath, Q129 emphysema PFT) were read via page-render vision for column accuracy. Answer keys read via vision from מפתח תשובות הרצת פרסום 101 (post-appeal authoritative): 36 multi-answer questions carried in c_accept (including 3 "כל התשובות מתקבלות" all-accept: Q29/Q60/Q119). Every question carries its canonical מראה מקום in the explanation (Harrison 22e page refs per the official source table; Q141-150 are landmark-trial refs: DIGIT-HF, AQUATIC, SELECT-GCA, HI-PRO, ECLIPSE, BALANCE, ZENITH, PREDMETH, ESCT-2, BAXHTN). Full clinical explanations authored via the AI pipeline (per-question reasoning + why-each-distractor-wrong) and topic-classified into the 24-topic taxonomy. Zero overlap with the existing 1556-question corpus (token-Jaccard dedup, max <0.6). data/.corpus_manifest.json regenerated (total 1706 + per-topic). Test count-locks + PAST_EXAM_TAGS + ALLOWED + per-tag EXPECTED map updated (regressionGuards, fractureRepair, parserBleedGuard). Trinity 10.4.58 → 10.4.59.',
  ],
  '10.4.58': [
  ...
  '10.4.57': [
    'fix(ai): give callAI specific, actionable error messages instead of a bare "API NNN" / dead שגיאה. New exported aiErrFromStatus(status) maps the HTTP status to a Hebrew-hinted message — 401 → "מפתח API לא תקין" (invalid key), 403 → "אין הרשאה למשאב/מודל זה" (permission_error: a VALID key lacking model/resource access), 429 → rate-limit, 5xx → service-unavailable — ported/extended from the Geriatrics _aiErrFromStatus helper. On a direct api.anthropic.com 401 (the path that uses the user\'s stored pnimit_apikey), callAI clears the stored key via setApiKey("") AND records a hash-only known-bad marker, so the user is re-prompted instead of looping on a dead key; a direct 403 KEEPS the key (it is a permission/billing/model-access problem, not a bad key — Codex P2). The login-restore path (features/auth.js) AND the legacy cloud-restore path (features/cloud.js applyRestorePayload) both skip re-writing a key matching the known-bad marker, and setApiKey() preserves the marker when re-saving the marked value — so an account-synced or backup-stored stale key no longer re-arms the loop on another device (Codex P2 round-2 on #191); the marker clears on a genuinely new key save or a successful AI call. A proxy 401/403 never clears the user key — the proxy authenticates with the shared x-api-secret — and the more-view.js chat handler (a pure proxy call) was corrected to stop wiping pnimit_apikey on a proxy 401/403, surfacing a service-unavailable message instead. Also wrapped the direct fetch in a try/catch so a network failure throws a clear "Network error" instead of a raw TypeError. Pinned by tests/aiClientErrorSpecificity.test.js (status→message split, 401-clears/403-keeps, proxy-401-keeps, known-bad-marker lifecycle) and tests/moreViewChatProxyKeyGuard.test.js; package-lock.json bumped to match. No question content, answer keys, explanations, source mappings, or audit queues changed. Trinity 10.4.56 to 10.4.57.',
  ],
  '10.4.56': [
