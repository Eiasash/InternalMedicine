...
export const CHANGELOG = {
  '10.4.57': [
    'fix(ai): give callAI specific, actionable error messages instead of a bare "API NNN" / dead שגיאה. New exported aiErrFromStatus(status) maps the direct-call HTTP status to a Hebrew-hinted message — 401/403 → "מפתח API לא תקין", 429 → rate-limit, 5xx → service-unavailable — ported from the Geriatrics _aiErrFromStatus helper. On a direct api.anthropic.com 401/403 (the path that uses the user\'s stored pnimit_apikey), callAI now clears the stored key via setApiKey("") so the user is re-prompted instead of silently looping on a dead key (matches the existing more-view.js chat handler). A proxy 401/403 is left untouched — the proxy uses the shared x-api-secret, not the user key. Also wrapped the direct fetch in a try/catch so a network failure throws a clear "Network error" instead of a raw TypeError. New tests/aiClientErrorSpecificity.test.js pins the status→message mapping and the 401/403-clears-key / 429-and-5xx-keep-key behavior. No question content, answer keys, explanations, source mappings, or audit queues changed. Trinity 10.4.56 to 10.4.57.',
  ],
  '10.4.56': [
