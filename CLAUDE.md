...
- **Current version**: v10.4.39 — fix(data): repair 11 more intra-word spaced-Hebrew Qs via surgical PURE de-spacing of UNAMBIGUOUS prefixes `ב/ל/מ/כ` cleaved from their word (e.g. `ב טיפול`→`בטיפול`, `מ בין`→`מבין`); provably space-only — 0 char/answer-key changes (1556 unchanged). `spacedHebrewGuard` detector extended to catch single-prefix splits (surfaced by Codex IM #157 P2). `ו`/`ה` are NOT auto-glued — ambiguous (`ו` word-final in `איזו`; `ה` a suffix in `מחלקה`/`באיזה`); gluing forward makes non-words (Codex IM #158 P2 caught 3). 14 `ו`/`ה`-ambiguous + scrambled cases quarantined for source-PDF reconstruction + sign-off. Prior: v10.4.38 — repaired 3 spaced-Hebrew Qs (415/743/800) + added the guard (ported from Geriatrics v10.64.145). Prior: v10.4.37 — fix(dark): hardcoded dark inline `color:#1e293b`/`#0f172a` collided 1:1 with the dark `.card`/body background → invisible; `theme.css` rescues them to light text under `body.dark`.
