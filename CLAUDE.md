...
- **Current version**: v10.4.38 — fix(data): repair intra-word spaced-Hebrew (PDF/BIDI extraction artifact) in 3 questions via surgical PURE de-spacing (only spaces removed, 0 char changes, 0 answer-key changes): idx 415 `ציטוגנטי ו ת`→`ציטוגנטיות`, 743 `מ י ימת`→`מיימת`, 800 `ת ו פע ו ת`→`תופעות`+`ל טיפול`→`לטיפול`. New `tests/spacedHebrewGuard.test.js` ratchet (ported from Geriatrics v10.64.145). 3 entangled cases (807/824/1544 — char-change/scrambled) quarantined in the allowlist pending source-PDF reconstruction (sign-off, like Geri #316). Q count unchanged (1556). Prior: v10.4.37 — fix(dark): hardcoded dark inline `color:#1e293b`/`#0f172a` collided 1:1 with the dark `.card`/body background → invisible; `theme.css` rescues them to light text under `body.dark`. Suite-wide P1 (companion to FamilyMedicine v1.25.6 + Geriatrics). Prior: v10.4.36 — code hygiene (invalid toast `direction:auto`, undefined skeleton vars, frozen `BUILD_HASH` removed).
